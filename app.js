...
const CF_PREF_KEYS = { 
  ...
  preferNew: "cf:random:preferNew" // 新增
};
...
function applyCFRandomPrefs() {
  ...
  const tagsEl = el("#cf-tags"); const countEl = el("#cf-count"); const handleEl = el("#cf-handle"); 
  const exSolvedEl = el("#cf-exclude-solved"); const preferNewEl = el("#cf-prefer-new"); // 新增
  ...
  // 新增：默认勾选优先新题
  if (preferNewEl) preferNewEl.checked = getPref(CF_PREF_KEYS.preferNew, "1") === "1"; 
  // 上面是你原有的恢复 checkbox 状态的代码...
  if (preferNewEl) preferNewEl.checked = getPref(CF_PREF_KEYS.preferNew, "1") === "1"; 

  // ====== 新增：联动显示逻辑 ======
  const wrapper = el("#cf-handle-wrapper");
  const handleInput = el("#cf-handle");
  const toggleHandle = () => {
    if (!exSolvedEl || !wrapper) return;
    if (exSolvedEl.checked) {
      wrapper.style.display = "block";
      // 展开时，如果账号为空，自动获取焦点方便直接输入
      ...
      // 清除可能残留的报错样式
      if(handleInput) handleInput.style.borderColor = "";
      if(el("#cf-handle-msg")) {
        el("#cf-handle-msg").textContent = "";
        el("#cf-handle-msg").style.color = "var(--muted)";
      }
    }
  };
  
  toggleHandle(); // 初始执行一次
  exSolvedEl.removeEventListener("change", toggleHandle); // 防止重复绑定
  exSolvedEl.addEventListener("change", toggleHandle);
}
...
function saveCFRandomPrefsFromForm() {
  ...
  if (preferNewEl) setPref(CF_PREF_KEYS.preferNew, preferNewEl.checked ? "1" : "0"); // 新增
}
...
// 注意参数列表中增加了 preferNew 选项
function pickRandomCF(problems, { ratingMin, ratingMax, tags, count, requireRated = false, excludeSolved = false, solvedSet = null, preferNew = false }) {
  let pool = problems.filter((p) => {
    ...
    if (tags && tags.length) { 
      const own = new Set((p.tags || []).map(t => t.toLowerCase())); 
  ...
  const targetCount = Math.max(1, Math.min(count || 1, 10));

  if (preferNew) {
    // 改进1：【软过滤】尝试剔除 1000 以前的远古题目（大约 2018 年以前的题）
    // 前提是剔除后，题库里还有足够的题目（至少是需要抽取数量的 3 倍，避免抽不到题）
    const RECENT_CUTOFF = 1000; 
    const recentPool = pool.filter(p => (p.contestId || 0) >= RECENT_CUTOFF);
    if (recentPool.length >= targetCount * 3) {
      pool = recentPool; // 题目充足，安全截断远古题
    }

    // 按 contestId 降序排列，最新的题目排在数组最前面 (index 较小)
    pool.sort((a, b) => (b.contestId || 0) - (a.contestId || 0));
    
    const result = [];
    for (let k = 0; k < targetCount; k++) {
      if (!pool.length) break;
      
      // 改进2：【加大权重】将 2 次方改为 4 次方
      // Math.random()^4 会让约 68% 的概率直接落在前 20% 的最新题目中，尾部概率被极致压缩
      const r = Math.random();
      const skewedIndex = Math.floor(Math.pow(r, 4) * pool.length);
      
      result.push(pool[skewedIndex]);
      pool.splice(skewedIndex, 1); // 抽出后从池中移除，避免重复抽取
    }
    return result;
  } else {
    // 传统的 Fisher-Yates 均匀洗牌逻辑
...
el("#cf-random-run").addEventListener("click", async (event) => {
  ...
  try {
    ...
    // 获取“优先新题”勾选状态
    const preferNew = !!el("#cf-prefer-new")?.checked;
    ...
    if (excludeSolved) {
      if (!handle) {
        if (handleInput && handleMsg) {
          handleInput.style.borderColor = "var(--danger, red)";
          handleMsg.textContent = "请填写账号后再抽取！";
          handleMsg.style.color = "var(--danger, red)";
          handleInput.focus();
        }
        return; // 直接 return，不弹 alert
      }
      // 账号不为空，恢复正常样式
      if (handleInput) handleInput.style.borderColor = "";
      ...
      if (!st.ok) {
        if (handleInput && handleMsg) {
          handleInput.style.borderColor = "var(--danger, red)";
          handleMsg.textContent = st.reason === "not_found" ? "账号不存在，请检查拼写" : "网络错误，请稍后再试";
          handleMsg.style.color = "var(--danger, red)";
          handleInput.focus();
        }
        return;
      ...
      if (handleMsg) {
        handleMsg.textContent = "账号校验通过";
        handleMsg.style.color = "var(--success, green)";
      }
    }
    ...
    // 传入 preferNew 参数
    const picked = pickRandomCF(problems, { ratingMin, ratingMax, tags, count, requireRated, excludeSolved, solvedSet, preferNew });
